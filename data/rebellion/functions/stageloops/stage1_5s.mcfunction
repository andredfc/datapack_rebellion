schedule function rebellion:stageloops/stage1_5s 5s replace

#12 cavalos +- durante stage 1
execute if predicate rebellion:05percent as @e[type=armor_stand,tag=center] at @s run summon horse ~ ~ ~ {Tame:1b,Tags:["fasthorse","horsespawned"],Attributes:[{Name:generic.max_health,Base:30},{Name:generic.movement_speed,Base:0.3},{Name:horse.jump_strength,Base:1.0}],SaddleItem:{id:"minecraft:saddle",Count:1b}}
execute if entity @e[type=horse,tag=horsespawned] as @e[type=armor_stand,tag=center] at @s run spreadplayers ~ ~ 2 250 false @e[type=horse,tag=horsespawned]
execute as @e[type=horse,tag=horsespawned] at @s run effect give @s regeneration 999999 0
